...
function heading_func($atts, $content = null){
	...
	<?php 
		...
		if ( strlen( $linkbox ) > 0 && strlen( $url['url'] ) > 0 ) {
			echo '<a href="' . esc_attr( $url['url'] ) . '" title="' . esc_attr( $url['title'] ) . '" target="' . ( strlen( $url['target'] ) > 0 ? esc_attr( $url['target'] ) : '_self' ) . '">'. $text .'</a>';
		}else{
			echo $text;
		} 
		echo '</'.$tag.'>';
   ...
   <div class="row">
      <?php 
        ...
        foreach ( $titles as $data ) {
            ...
            $img = $img[0];
      ?>
      	<div class="<?php echo implode( ' ', $css_classes ); ?> text-middle text-light" <?php if($animation != 'disable'){ ?>data-wow-delay="<?php echo esc_attr($i*$animate_time); ?>s"<?php } ?> >
...
?>	
	...
	<div class="center-y text-center">
        <div class="spacer-double"></div>
        <?php if($title != ''){ ?>
	        <h1 class="<?php echo esc_attr($title_extraclass); ?>"><?php echo htmlspecialchars_decode($title) ?></h1>
	    <?php  }  ?>         
		<div class="spacer-half"></div>	

		<?php if($stitle != ''){ ?>        	
	        <span class="<?php echo esc_attr($subtitle_extraclass); ?>"><?php echo htmlspecialchars_decode($stitle) ?></span>
	        <div class="spacer-double"></div>
        <?php } ?>	
            ...
            <div class="row">
                <div class="col-md-7">
					...
					<div class="text-slider border-deco <?php echo esc_attr($extraclass); ?>">
						...
						?>					   
							<div class="text-item">
								<?php if($data['stitle'] != ''){ ?><h3><span class="id-color"><?php echo htmlspecialchars_decode($data['stitle']); ?></span></h3><?php } ?>
								<?php if($data['title'] != ''){ ?><h2 class="style-4"><?php echo htmlspecialchars_decode($data['title']); ?><span class="id-color">.</span></h2><?php } ?>
							</div>  
						<?php } ?>
                    </div>
					<div class="spacer-double"></div>
...
function text_slider_box_func($atts, $content = null){
	...
	<?php if($logo != ''){ ?>
		...
		<div id="logo">
			<a href="<?php echo esc_url( home_url('/') ); ?>">
				<img class="logo" src="<?php echo esc_url($img); ?>" alt="">
			</a>
		</div>
		<!-- logo close -->
		<div class="spacer-double"></div>
	...
	<div class="text-slider border-deco <?php echo esc_attr($extraclass); ?>">
		...
		?>					   
			<div class="text-item">				
				<?php if($data['title'] != ''){ ?><h2 class="style-3"><?php echo htmlspecialchars_decode($data['title']); ?><span class="id-color">.</span></h2><?php } ?>
			</div>  
		<?php } ?>
    </div>       
...
function ot_before_after_func($atts, $content = null){
	...
	<?php 		
		foreach ( $titles as $data ) {
			...
			$img_before = wp_get_attachment_image_src($data['beforeimg'],'full');
			$img_before = $img_before[0];

			$img_after = wp_get_attachment_image_src($data['afterimg'],'full');
			$img_after = $img_after[0];
			...
			<?php if ($data['layoutstyle'] == 'imgright') { ?>
				...
				<div class="col-md-6 text-middle">
					<div class="twentytwenty-container">
					  <img src="<?php echo esc_url($img_before); ?>" alt="" class="img-responsive" />							  
					  <img src="<?php echo esc_url($img_after); ?>" alt="" class="img-responsive" />
					</div>
				</div>
			<?php }else{ ?>
				<div class="col-md-6 text-middle">
...
function ot_before_after_slider_func($atts, $content = null){
	...
	<!-- Element: OT Before After Box -->
	<div class="owl-carousel carousel-single-navi before-after-slider <?php echo esc_attr($extraclass); ?>">
		<?php 		
			foreach ( $titles as $data ) {			
				$data['beforeimg'] = isset( $data['beforeimg'] ) ? $data['beforeimg'] : '';
				$data['afterimg'] = isset( $data['afterimg'] ) ? $data['afterimg'] : '';			
		...
		?>	
			<div class="twentytwenty-container shadow-soft">
				<img src="<?php echo esc_url($img_before); ?>" class="img-responsive img-rounded" />							  
				<img src="<?php echo esc_url($img_after); ?>" class="img-responsive img-rounded" />
			</div>					
		<?php } ?> 
	</div>      
    <script type="text/javascript">						
...
function home_youtubevideo_func($atts, $content = null){
    ...
    <div id="section-video-bg" class="full-height no-padding" data-speed="5" data-type="background">
        <div class="de-video-container">
			...
			<a id="bgndVideo" class="player" data-property="{videoURL:'<?php echo esc_attr( $video_link ); ?>',containment:'#section-video-bg',autoPlay:<?php echo esc_attr( $autoplay ); ?>, mute:<?php echo esc_attr( $muted ); ?>, startAt:5, opacity:1}"><?php esc_attr_e('youtube', 'archi'); ?></a>
        </div>
    </div>
...
<div class="row">
	...
	?>
		...
		<div class="<?php if ($services_columns == 2) {echo 'col-md-6'; }elseif ($services_columns == 3) { echo 'col-md-4'; }else{echo 'col-md-3';} ?> text-middle text-light <?php if($servicebox_animation == ''){ echo 'wow fadeInRight'; } ?>" data-wow-delay="<?php echo esc_attr($i); ?>s">
